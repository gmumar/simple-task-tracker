...
import { ChevronLeft, ChevronRight, CheckCircle2, XCircle, Ban, Trophy, Undo2, Plus, X, Trash2, Play } from 'lucide-react';
...
export default function WeekView({ currentDate, setCurrentDate, tasks, onTasksUpdated }) {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'in-progress': return '#af52de'; // Purple for in-progress
  ...
  const tasksByStatus = useMemo(() => {
    ...
    tasks.forEach(task => {
      const groupKey = task.status === 'in-progress' ? 'open' : task.status;
      if (groups[groupKey]) {
                    ...
                    <AnimatePresence mode="popLayout">
                      {tasksByStatus[status.id].length === 0 ? (
                        ...
                        tasksByStatus[status.id].map(task => {
                          ...
                          const isClosed = task.status !== 'open' && !isInProgress;
                            ...
                            >
                              ...
                              <div className={styles.taskActionsSide}>
                                {!isClosed ? (
                                  <>
                                    <button
                                      className={`${styles.taskBtn} ${isInProgress ? styles.btnInProgressActive : styles.btnInProgress}`}
                                      onClick={(e) => { e.stopPropagation(); handleStatusChange(task.id, isInProgress ? 'open' : 'in-progress'); }}
                                      title={isInProgress ? "Stop Progress" : "Start Progress"}
                                    >
                                      <Play size={14} fill={isInProgress ? "currentColor" : "none"} />
                                    </button>
